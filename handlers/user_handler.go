...
func GetAllUsers(ctx *fiber.Ctx) error {
	...
	result := database.UsingPostgre.Find(&users)
	if result.Error != nil {
...
// 	errCreateUser := database.UsingPostgre.Create(&newUser).Error
...
func GetUserId(ctx *fiber.Ctx) error {
	...
	err := database.UsingPostgre.First(&user, userId).Error
	if err != nil {
...
// 	var user models.User
// 	if err := database.UsingPostgre.First(&user, id).Error; err != nil {
// 		return ctx.Status(404).JSON(fiber.Map{
...
// 	database.UsingPostgre.Save(&user)
...
func DeleteUserId(ctx *fiber.Ctx) error {
	...
	// Soft delete
	err := database.UsingPostgre.Delete(&user)
	if err != nil {
